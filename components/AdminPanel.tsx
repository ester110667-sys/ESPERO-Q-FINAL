...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  // Estado interno para eventos com dados completos de inscritos, exclusivo para o admin
  ...
  const loadAdminData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Falha ao carregar dados do admin:", e);
    } finally {
  ...
  useEffect(() => {
    ...
    // Carrega dados e se inscreve para atualizações em tempo real
    loadAdminData();
    const unsub = api.subscribeToChanges(() => loadAdminData());

    return () => unsub(); // Limpa a inscrição ao desmontar
  }, [loadAdminData]);
  ...
  const handleRemoveEvent = async (id: string) => {
    ...
    try {
      ...
      await loadAdminData(); // Força a atualização imediata
    } catch (err: any) {
      alert(err.message);
    } finally {
