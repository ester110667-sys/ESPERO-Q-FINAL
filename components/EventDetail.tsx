...
const EventDetail: React.FC<EventDetailProps> = ({ event, currentTime, onBack, onRegister }) => {
  ...
  useEffect(() => {
    // Resetar estado ao trocar de evento
    setRegistrants([]);
    setPage(1);
    setHasMore(true);
    initialLoad();
  }, [event.id, initialLoad]);

  const loadMoreRegistrants = async () => {
    if (isLoadingList || !hasMore || loadingRef.current) return;
    loadingRef.current = true;
    setIsLoadingList(true);
    try {
  ...
  const maskEmail = (emailStr: string) => {
    ...
    const maskedUser = user.length > 2 ? `${user.charAt(0)}***${user.charAt(user.length - 1)}` : '***';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Recarregar lista para mostrar o novo inscrito
      initialLoad();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="group flex items-center gap-3 px-6 py-3 bg-zinc-900/50 hover:bg-zinc-800 border border-zinc-800 rounded-full text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-white transition-all active:scale-95">
          <span className="group-hover:-translate-x-1 transition-transform">←</span> Retornar
        </button>
      ...
      <section className="bg-zinc-950 border border-zinc-900 rounded-[3rem] overflow-hidden shadow-2xl relative bg-zinc-900">
        <div className="relative aspect-video sm:aspect-[21/9]">
          <img src={event.imageUrl} className={`w-full h-full object-cover transition-opacity duration-1000 ${isFinished ? 'grayscale' : 'grayscale-[10%]'}`} alt={event.name} fetchPriority="high" />
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent" />
          <div className="absolute bottom-8 left-8 right-8">
            <h1 className="text-3xl sm:text-5xl font-black uppercase tracking-tighter leading-none mb-4 drop-shadow-2xl">{event.name}</h1>
      ...
      <section className={`bg-zinc-950 border border-zinc-900 rounded-[3rem] p-8 sm:p-12 relative shadow-2xl transition-all ${isFinished ? 'opacity-60' : ''}`}>
        ...
        {message && <div className={`mb-8 p-5 rounded-2xl border text-xs font-bold animate-in zoom-in-95 duration-300 ${message.type === 'success' ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>{message.text}</div>}
        ...
        {isFinished ? (
          ...
          <div className="p-12 text-center bg-zinc-900/20 rounded-[2rem] border border-orange-500/10 animate-pulse">
            <p className="text-white font-black uppercase tracking-widest text-sm">Sua vaga está garantida!</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} disabled={isFull || !isAfterOpen || isSubmitting} placeholder="Seu Nome Completo" className="w-full bg-zinc-900 border border-zinc-800 focus:border-orange-500 rounded-2xl px-6 py-4 text-white outline-none font-bold text-sm disabled:opacity-30 transition-all placeholder:text-zinc-700" />
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={isFull || !isAfterOpen || isSubmitting} placeholder="Seu E-mail Profissional" className="w-full bg-zinc-900 border border-zinc-800 focus:border-orange-500 rounded-2xl px-6 py-4 text-white outline-none font-bold text-sm disabled:opacity-30 transition-all placeholder:text-zinc-700" />
            </div>
            <button type="submit" disabled={isFull || !isAfterOpen || isSubmitting} className={`w-full py-5 font-black uppercase tracking-[0.3em] rounded-2xl transition-all shadow-2xl active:scale-[0.98] text-xs border ${isFull || !isAfterOpen || isSubmitting ? 'bg-zinc-900 text-zinc-700 border-zinc-800 cursor-not-allowed' : 'bg-orange-600 hover:bg-orange-500 text-white border-orange-600 shadow-orange-900/20'}`}>
      ...
      <section className="bg-zinc-950 border border-zinc-900 rounded-[3rem] p-8 sm:p-12 shadow-2xl">
        <h2 className="text-xl font-black uppercase tracking-tight mb-8 border-b border-zinc-900 pb-6 flex items-center justify-between">
          ...
          <span className="text-xs bg-zinc-900 px-3 py-1 rounded-full text-zinc-500 border border-zinc-800">{event.registrantCount || registrants.length}</span>
        </h2>
        
        {registrants.length === 0 && !isLoadingList ? (
          <div className="py-12 text-center border-2 border-dashed border-zinc-900 rounded-[2rem]">
            <p className="text-zinc-800 text-[10px] font-black uppercase tracking-widest italic">Aguardando as primeiras inscrições...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3">
            {registrants.map((reg, idx) => (
              <div key={reg.id} className="flex items-center justify-between gap-4 bg-zinc-900/20 border border-zinc-800/50 p-4 rounded-2xl group transition-all hover:bg-zinc-900/40 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-4">
                  <span className="text-xs font-black text-orange-500/40 w-4">{idx + 1}</span>
                  <span className="font-black text-zinc-300 text-sm uppercase tracking-tight">{reg.name}</span>
                </div>
                <span className="text-[10px] font-mono text-zinc-700 hidden sm:inline-block">{maskEmail(reg.email)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {hasMore && !isLoadingList && registrants.length > 0 && (
          <div className="mt-8 text-center">
            <button onClick={loadMoreRegistrants} className="px-8 py-3 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-xl text-[9px] font-black uppercase border border-zinc-800 transition-all active:scale-95">
              Carregar Mais
            </button>
          </div>
        )}
