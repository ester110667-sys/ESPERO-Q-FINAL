...
const EventDetail: React.FC<EventDetailProps> = ({ event, currentTime, onBack, onRegister }) => {
  ...
  useEffect(() => {
    // Carrega a primeira página de inscritos ao montar o componente
    setRegistrants([]);
    setPage(1);
    setHasMore(true);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 max-w-4xl mx-auto">
      ...
      <section className="bg-zinc-950 border border-zinc-900 rounded-[3rem] overflow-hidden shadow-2xl relative bg-zinc-900">
        <div className="relative aspect-video sm:aspect-[21/9]">
          <img src={event.imageUrl} className={`w-full h-full object-cover ${isFinished ? 'grayscale' : 'grayscale-[20%]'}`} alt={event.name} onError={(e) => (e.currentTarget.style.opacity = '0')} />
      ...
      <section className={`bg-zinc-950 border border-zinc-900 rounded-[3rem] p-8 sm:p-12 relative ${isFinished ? 'opacity-60' : ''}`}>
        ...
        {isFinished ? <div className="p-12 text-center bg-zinc-900/20 rounded-[2rem] border border-zinc-800 text-zinc-500 font-black uppercase tracking-widest text-sm">Evento Encerrado</div> : isAlreadyRegisteredBrowser ? <div className="p-12 text-center bg-zinc-900/20 rounded-[2rem] border border-orange-500/10"><p className="text-white font-black uppercase tracking-widest text-sm">Sua vaga está garantida!</p></div> : <form onSubmit={handleSubmit} className="space-y-6"><div className="grid grid-cols-1 sm:grid-cols-2 gap-6"><input type="text" value={name} onChange={(e) => setName(e.target.value)} disabled={isFull || !isAfterOpen || isSubmitting} placeholder="Seu Nome Completo" className="w-full bg-zinc-900 border border-zinc-800 focus:border-orange-500 rounded-2xl px-6 py-4 text-white outline-none font-bold text-sm disabled:opacity-30" /><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={isFull || !isAfterOpen || isSubmitting} placeholder="Seu E-mail Profissional" className="w-full bg-zinc-900 border border-zinc-800 focus:border-orange-500 rounded-2xl px-6 py-4 text-white outline-none font-bold text-sm disabled:opacity-30" /></div><button type="submit" disabled={isFull || !isAfterOpen || isSubmitting} className={`w-full py-5 font-black uppercase tracking-[0.3em] rounded-2xl transition-all shadow-2xl active:scale-[0.98] text-xs border ${isFull || !isAfterOpen || isSubmitting ? 'bg-zinc-900 text-zinc-700 border-zinc-800 cursor-not-allowed' : 'bg-orange-600 hover:bg-orange-500 text-white border-orange-600'}`}>{isSubmitting ? "Processando..." : (isFull ? 'Vagas Esgotadas' : (!isAfterOpen ? 'Inscrições em Breve' : 'Garantir Minha Vaga'))}</button></form>}
      </section>

      {event.registrantCount > 0 && (
        <section className="bg-zinc-950 border border-zinc-900 rounded-[3rem] p-8 sm:p-12 shadow-2xl">
          <h2 className="text-xl font-black uppercase tracking-tight mb-8 border-b border-zinc-900 pb-6">Fotógrafos <span className="text-orange-500">Confirmados</span> ({event.registrantCount})</h2>
          <div className="grid grid-cols-1 gap-3">
            {registrants.map((reg, idx) => (
              <div key={reg.id} className="flex items-center justify-between gap-4 bg-zinc-900/20 border border-zinc-900 p-4 rounded-2xl group transition-all hover:bg-zinc-900/40">
                <div className="flex items-center gap-4"><span className="text-xs font-black text-orange-500/40">{idx + 1}</span><span className="font-black text-zinc-300 text-sm uppercase">{reg.name}</span></div>
